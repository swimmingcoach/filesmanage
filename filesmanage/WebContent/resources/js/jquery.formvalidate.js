...
(function($) {
	...
	String.prototype.sprintf = function(params) {
		...
		$.each(params, function(index, param) {
			try {
				if ($("#"+param).attr("title")) {
					string = string.replace('{' + index + '}', $("#"+param).attr("title"));	
				} else {
	...
	$.fn.formvalidate.options = {
		...
		_process: function(form, options) {
			...
			$(form).find(':input').each(function(index, element) {
				...
				$.each(options.validations, function(validation, method) {
					...
					//If this is the required_either validation
					else if (validation === 'requiredEither') {
						if ('requiredEither' in data) {
							inputs[attrName].validations.requiredEither = [data.requiredEither];
						}
					}
					...
					$.each(inputObj.validations, function(validationName, validationParams) {
						...
						if (inputObj.failure === null) {
							...
							if (validationName in options.validations) {
								...
								if (inputObj.value === null && validationName !== 'required' && validationName !== 'requiredIf' && validationName !== 'requiredEither') {
		...
		validations: {
			...
			url: function(input, params) {
				return input.match(/^https?:\/\/[A-Za-z0-9\-_]+\.[A-Za-z0-9\-_]+[\/=\?%\-&_~`@[\]':+!]*([^<>\"\"])*$/) == null ? false : true;
			},
			...
			requiredEither: function(input, params) {
				var el = $('#'+params[0]);
				var elHasVal = false;
				if ($(el).is(':checkbox') || $(el).is(':radio')) {
					elHasVal = $(el).is(':checked') ? true : false;
				} else {
					elHasVal = $.trim( $(el).val() ) === '' ? false : true;
				}
				var selfHasVal = false;
				if ($.trim(input) != "") {
					selfHasVal = true;
				}
				return elHasVal || selfHasVal;
			},
			equalsInput: function(input, params) {
				var value1 = $('#'+params[0]).val();
				if (input == value1) {
					return true;
				} else {
					return false;
